...
class _TritronFusedAddRMSNorm(torch.autograd.Function):
    ...
    def forward(ctx, hidden_states, old_residual, weight, eps=1e-6):
        ...
        rms_std = torch.empty(M, dtype=torch.float32, device=hidden_states.device)
        
        num_warps=8
        num_stages=1
        _add_rmsnorm_fwd[(M, )](hidden_states, old_residual, weight, new_residual, output, rms_std, eps,
                            *hidden_states.stride(),
                            BLOCK_N,
                            N,
                            num_warps=num_warps, num_stages=num_stages)
        ctx.save_for_backward(hidden_states, new_residual, weight, rms_std)
    ...
    def backward(ctx, dy, dnew_res):
        ...
        hidden_states, new_residual, weight, rms_std = ctx.saved_tensors
        M,N = hidden_states.shape
...
class TritonFusedAddRMSNorm(torch.nn.Module):
    ...
    def forward(self, hidden_states, old_residual):
        return triton_fused_add_norm(hidden_states, old_residual, self.weight, self.eps)