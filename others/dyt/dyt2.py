...
# @triton.autotune([triton.Config({"BLOCK_M":bn}, num_stages=ns, num_warps=nw)
...
def _dyt_fwd_kernel(X,
    ...
    off_m = tl.cast(tl.program_id(0), tl.int64) * BLOCK_M + tl.arange(0, BLOCK_M)
    off_n = tl.cast(tl.program_id(1), tl.int64)

    alpha = tl.load(Alpha).to(tl.float32)
    
    gemma = tl.load(Gemma + off_n).to(tl.float32)  
    beta = tl.load(Beta + off_n).to(tl.float32) 
    x = tl.load(X + off_m * N + off_n, mask=off_m < M, other=0.).to(tl.float32) 

    tanh_x = tanh(alpha * x)
    y = tanh_x * gemma + beta
    tl.store(Y + off_m * N + off_n, y, mask=off_m < M)
...
class _DYT(torch.autograd.Function):
    ...
    def forward(ctx, x, alpha, gemma, beta):
        ...
        kwargs = {"BLOCK_M": min(triton.next_power_of_2(M), 4096), "num_warps":4, "num_stages": 2}
    ...
    def backward(ctx, dy):
        ...
        da = torch.zeros(1, dtype=torch.float32, device=x.device)
        dg = torch.empty(N, dtype=torch.float32, device=x.device)
        db = torch.empty(N, dtype=torch.float32, device=x.device)

        kwargs = {"BLOCK_M": min(triton.next_power_of_2(M), 2048), "num_warps":4, "num_stages": 1}
        grid = lambda meta: (N)
