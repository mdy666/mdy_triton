...
from . import comm as p2p_communication
...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config, deallocate_pipeline_outputs=None):
    ...
    if deallocate_pipeline_outputs is None:
        deallocate_pipeline_outputs = config.deallocate_pipeline_outputs
...
def start_sync_grad(model):
    for param in model.param_to_bucket_group:
        if model.ddp_config.overlap_grad_reduce:
            model.param_to_bucket_group[param].register_grad_ready(param)
...
def zb2p_v2(
    *,
    ...
    CustomManger.store_wgrad_func = True
    num_linear = 0

    current_microbatch = 0
    step1 = (num_ranks - rank - 1) * 2
    for i in range(step1):
        ...
        if not forward_only:
            if output_tensors:
                deallocate_output_tensor(output_tensors[-1][0], config.deallocate_pipeline_outputs)
        output_tensor, num_tokens = forward_step(
            forward_step_func,
            data_iterator,
            model,
        ...
        # if rank == 0 and i == step1 - 1:
        #     BackwardManger.wait_send()
        #     deallocate_output_tensor(output_tensor[0], config.deallocate_pipeline_outputs)

        if not forward_only:
            input_tensors.append(input_tensor)
            output_tensors.append(output_tensor)
        # print(f"<step1: {step1}, rank: {rank}, id: {i}, {num_microbatches}")
    ...
    for i in range(step2):
        ...
        output_tensor, num_tokens = forward_step(
            ...
            checkpoint_activations_microbatch,
            check_first_val_step(
                first_val_step, forward_only, (i == 0) and (step1 == 0)
            ),
            current_microbatch=current_microbatch,
            encoder_decoder_xattn=encoder_decoder_xattn,
        )
        ...
        if forward_only:
            ...
            # Pop input_tensor and output_tensor from the start of the list for
            # the backward pass.
            input_tensor_2 = input_tensors.pop(0)
            output_tensor_2 = output_tensors.pop(0)

            
            input_tensor_grad = backward_step(
                input_tensor_2, output_tensor_2, output_tensor_grad, model_type, config, deallocate_pipeline_outputs=True
            )   
        ...
        if i == 0:
            num_linear = len(CustomManger.wgrad_func)
        # print(f"<step1: {step1}, rank: {rank}, id: {i}, {num_microbatches}")
    

        

    step3 = num_microbatches - step1 - step2
    for i in range(step3):
        last_iteration = i == (step3 - 1)
        
        checkpoint_activations_microbatch = None
        last_iteration = current_microbatch == (num_microbatches - 1)
        ...
        output_tensor, num_tokens = forward_step(
            ...
            checkpoint_activations_microbatch,
            check_first_val_step(
                first_val_step, forward_only, False,
            ),
            current_microbatch=current_microbatch,
            encoder_decoder_xattn=encoder_decoder_xattn,
        )
        ...
        if forward_only:
            ...
            for _ in range(num_linear):
                CustomManger.weight_backward()
            
            send_forward(output_tensor, send_tensor_shapes, config)
            if last_iteration:
                CustomManger.wait_send()
                deallocate_output_tensor(output_tensor[0], config.deallocate_pipeline_outputs)


            # print(f"<step3: {step3}, rank: {rank}, id: {i}, {num_microbatches}")
    
    if not forward_only:
        step4 = step1
        for i in range(step4):
            ...
            if i == (step4 - 1):
                if config.grad_sync_func is None or rank == 0:
                    CustomManger.store_wgrad_func = False
                    enable_grad_sync()
            ...
            for _ in range(num_linear):
                CustomManger.weight_backward()
            # print(f"<step4: {step4}, rank: {rank}, id: {i}, {num_microbatches}")


        # step 5
        while CustomManger.wgrad_func:
            CustomManger.weight_backward()

        assert len(CustomManger.wgrad_func) == 0
